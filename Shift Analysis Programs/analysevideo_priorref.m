function [datafilename,referenceimage_prior] = analysevideo_priorref(videoname,refframefilename,blinkframefilename,...
...
stripidx = stripidx(:);
if stripidx(end)<512
    stripidx(length(stripidx)+1)=512;
end
...
%         asi_shifts(framecounter,:)=shifts;
        if peaks_noise(3) == 0
            peaks_noise(3) = 1;
        end
...
% goodmatches=find(peakratios_strips<=0.65);
if numframesforanalyses>1
    for directioncounter = 1:2
        ...
        interp_yaxis = interp1(sample_xaxis,tempshifts(goodmatches),interp_xaxis,'spline','extrap');
        frameshifts_strips_spline(:,directioncounter) = interp_yaxis;
%         asi(goodmatches)=interp_yaxis((goodmatches));
%         asi(setdiff(1:9900,goodmatches))=NaN;
% %         asiOut=find(isoutlier(asi,'movmedian',10)==1);
% %         asii=interp_yaxis((goodmatches));
% %         asii(setdiff(1:9900,asiOut))=NaN;
% %         asi(asiOut)=NaN;
%         as1=fillmissing(asi,'spline');
%         as2=fillmissing(asi,'pchip');
%         as3=fillmissing(asi,'makima');
%         ast(:,directioncounter)=interp_yaxis;
% %         %%
%         asi(isnan(asi))=1;
%         frameshifts_strips_spline(:,directioncounter) = asi;
    end
%     asi1 = frameshifts_strips_spline;
...
%         end
%         nnn=1
%         asi2 = frameshifts_strips_spline;
%     end
%     if min(badmatches(:)) == 1
%         firstgoodsample = min(goodmatches(:));
%         for directioncounter = 1:2
...
%         end
%         nnn=2
%         asi3 = frameshifts_strips_spline;
%     end
    
else
    warning('Unable to stabilize video')
end

close(analysisprog);

% clear old data
% delete(refframefilename)
% frameshifts_strips_spline(:,1)=reshape(frameshifts_strips_unwraped(:,:,1),[33*length(frameshifts_strips_unwraped),1]);
% frameshifts_strips_spline(:,2)=reshape(frameshifts_strips_unwraped(:,:,2),[33*length(frameshifts_strips_unwraped),1]);
% save('asi1.mat','frameshifts_strips_spline')
% asiGood=find(peakratios_strips<=0.6);
% frameshifts_strips_spline(setdiff(1:length(frameshifts_strips_spline),asiGood),:)=NaN;
% frameshifts_strips_spline(:,1)=fillmissing(frameshifts_strips_spline(:,1),'spline');
% frameshifts_strips_spline(:,2)=fillmissing(frameshifts_strips_spline(:,2),'spline');
% save('asi2.mat','frameshifts_strips_spline')
...
save(datafilename,'referenceimage','referencematrix','referencematrix_full',...
    ...
    'timeaxis','timeaxis_secs','windowflag','tempshifts','goodmatches','stripsalreadyanalysed_unwraped','frameshifts_strips_unwraped');
