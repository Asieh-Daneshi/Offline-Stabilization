...
minimummeanlevel = 2; % 8; %(ND)    % This is minimum mean pixel value that a frame has to
...
peakratiodiff = .5;   % from 0.1   % Maximum Change in ratio between the secondpeak and firstpeak
...
maxmotionthreshold = 0.0512; % 0.3;(ND) % Maximum motion between frames, expressed as a percentage
...
priorref_inputstruct = struct('samplerate',[],'vertsearchzone',50,'stripheight',9,...
    'badstripthreshold',0.85,'frameincrement',10,'minpercentofgoodstripsperframe',0.4,...
    'numlinesperfullframe',numlinesperfullframe); % Structure that contains input arguments for the makereference_priorref.m function.


%%
analyse_inputstruct = struct('samplerate',[],'vertsearchzone',50,'horisearchzone',[],...
    'startframe',1,'endframe',-1,'stripheight',9,'badstripthreshold',0.85,'minpercentofgoodstripsperframe',0.4,...
...
%             DispFramesFig = figure('Position',[screensize(3)/2-800 screensize(4)/2-600 1600 1200]);
            Frames = 1:300;
            corrValue = 279;
            for ax = 1+corrValue:6+corrValue
                subplot(2,3,ax-corrValue), imshow(double(read(VObj1,Frames(ax)))./255);
                axis off, box off
                title(sprintf('Frame # %02.0f',Frames(ax)))
                Names{ax-corrValue} = num2str(Frames(ax));
            end
...
%             close(DispFramesFig)
            ...
            PreRefIm = double(read(VObj1,Frames(selection3+corrValue)));
            ...
            if SyPRI~=712
                if SyPRI>712
                    ...
                    %crop
                    ReferenceImage = PreRefIm(VCenter-245:VCenter+266,HCenter-255:HCenter+256);  % test with smaller cutout
                    ImageSelection = false;
                end
            elseif size(PreRefIm,1)==712
...
%         figure,imshow(finerefimage./255)
        ...
        if GlobalReference
            analyseddatafilename = analysevideo_priorref(finalname, finereffilename, blinkfilename,...
                'referenceimage_prior', analyse_inputstruct, analyse_programflags,correlationflags_analyse, [0,0,0,0]);
        else
            analyseddatafilename = analysevideo_priorref(finalname, finereffilename, blinkfilename,...
                'referenceimage', analyse_inputstruct, analyse_programflags,correlationflags_analyse, [0,0,0,0]);
        end
