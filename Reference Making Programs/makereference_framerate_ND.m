function [datafilename,referenceimage] = makereference_framerate_ND(videoname,badframefilename,...
    frameincrement,badsamplethreshold,correlationflags,verbosityarray,RefImSelection,RefImInput)
...
% RefImSelection            - Added later: A string, either 'global' or 'individual'
% RefImInput                - Added later: If RefImSelection is 'global', the global reference image has to be inserted manually
%
...
if (nargin < 1) || isempty(videoname)
    ...
    if ischar(videoname)
        if exist(videoname,'file')
            [videopath,name,ext] = fileparts(videoname);
            videofilename = strcat(name, ext);
...
% set default values for frameincrement and badsamplethreshold
...
% get video parametrs if necessary
if toanalysevideo
...
% check whether frameincrement is too low 
if (frameincrement < 3)
    warning('Frame increment is too low, increasing to 3');
    frameincrement = 3;
end
% check whether frameincrement is too high
...
% load blink frame data if necessary
...
% show test matrices during correlation if necessary
...
% set default values before the algorithm
...
% set global reference if it was given to the function
if strcmp(RefImSelection,'global')
    referenceimage = RefImInput;
    GlobalReference = RefImInput;
% otherwise get first frame to analyse
else
...
% show Cross-Correlation figure if necessary
...
% read the video to analyse
if toanalysevideo
    vidObject4 = VideoReader(videoname);
end
% vidObject4CurrentTime = vidObject4.CurrentTime;


while ~toexit
    % badmatchesthatcorrelated = [];
    badmatchesthatcorrelated = zeros(numframesforcurrentlevel, 1);
    badMatchesThatCorrelatedIndex = 1;
    waitbar(0,analysisprog,'Correlating Frames');
    
    % set default current time
    % vidObject4.CurrentTime = vidObject4CurrentTime;
    
    % analyse each selected frame
    for framecounter = 1:numframesforcurrentlevel
        ...
        % read frame
        if toanalysevideo
            % set current time to video object to read frame
        ...
        if peakratio < badsamplethreshold
            badmatchesthatcorrelated(badMatchesThatCorrelatedIndex) = indexintomatrices;
            badMatchesThatCorrelatedIndex = badMatchesThatCorrelatedIndex + 1;
    ...
    % delete preallocated zeros
    badmatchesthatcorrelated = badmatchesthatcorrelated(badmatchesthatcorrelated~=0);
...
save(datafilename,'referenceimage','GlobalReference','referencematrix','referencematrix_full',...
