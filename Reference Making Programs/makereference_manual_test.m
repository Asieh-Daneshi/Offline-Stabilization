function [datafilename,referenceimage] = makereference_manual_test(videoname,badframefilename,frameincrement,...
...
if toanalysevideo
    ...
    if strcmp(videotype,'truecolor') || length(size(double(frame2im(readFrame(videoinfo))))) >= 3
...
if toanalysevideo    
    ...
    referenceframe = double(frame2im(readFrame(videoinfo))) / 256;    
else
    referenceframe = videoname(:,:,framesforreference(1)) / 256;
end
referenceframe = cat(3,referenceframe,zeros(size(referenceframe )),referenceframe );

framefigurehandle = figure;
frameimagehandle = image(referenceframe);
frameaxishandle = gca;
...
for framecounter = 2:numframesforreference
    ...
    if toanalysevideo
        ...
        currframetoimage = videoname(:,:,currframenumbertoimage);
        prevframetoimage = videoname(:,:,prevframenumbertoimage);
    end
    
    currframetoimage = scale(currframetoimage);
    
    currframetoimage_thumbnail = makethumbnail(currframetoimage,thumbnailfactor,thumbnailfactor);
    referenceframe_thumbnail = makethumbnail(referenceframe,thumbnailfactor,thumbnailfactor);
%     prevframetoimage_thumbnail = makethumbnail(prevframetoimage,thumbnailfactor,thumbnailfactor);

    [correlation thumbnailshifts thumbnailpeaks_noise] = corr2d(referenceframe_thumbnail,currframetoimage_thumbnail);
    ...
    baseframe = cat(3,scale(referenceframe),zeros(size(referenceframe)),zeros(size(referenceframe)));
    ...
    [referencematrix,referencematrix_full] = makestabilizedframe(videoname,framesforreference(1:framecounter),...
        frameshifts(1:framecounter,:),peakratios(1:framecounter),stripidx,badcorrelthreshold,frameheight + 1,...
        maxsizeincrement,stabsplineflag,stabverbose);

    referenceframe = referencematrix(:,:,2);
end
...
[referencematrix,referencematrix_full] = ...
    ...
    stripidx,badcorrelthreshold,frameheight + 1,maxsizeincrement,stabsplineflag,stabverbose);
