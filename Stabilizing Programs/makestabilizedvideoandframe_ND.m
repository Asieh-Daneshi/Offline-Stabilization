function [stabilisedvideoname,stabilisedframe,stabilisedframe_full] =...
    makestabilizedvideoandframe_ND(videoname,framestostabilize,frameshifts,...
...
% changed lines (Niklas Domdei, 08th March 2019)
% a) to be constant across videos 
% b) to match the size of ICANDI stabilized videos
stabilizedsize = [712 712];
imageborders = [1 712 1 712];
% ----------------------------------------------------------------------------------------------------------------
    ...
    if splineflag
        ...
        for framecounter = 1:numanalysedframes
            ...
            for directioncounter = 1:2
                ...
                interp_yaxis = interp1(sample_xaxis,sample_yaxis,interp_xaxis,'spline','extrap');
...
if toanalysevideo
    stabilisedvideoname = strcat(videoname(1:end - 4),'_stab_A.avi');
else
...
stabilisedvideoobject = VideoWriter(stabilisedvideoname, 'Grayscale AVI');
...
for framecounter = 1:numbervideoframes
    ...
    writeVideo(stabilisedvideoobject, frametoadd);
