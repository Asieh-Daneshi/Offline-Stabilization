function varargout = makestabilizedvideo(videoname,...
    framestostabilize,frameshifts,peakratios,stripindices,badsamplethreshold,...
    maintaintimerelationships,numlinesperfullframe,blacklineflag,maxsizeincrement,splineflag,verbose)
...
%
% stabilisedvideoname       - A optional string that contains the full path
...
[sizeincrement,stabilizedsize,imageborders] = getstabilizedparamsfixedframesize(frameshiftstouse,framesizes,maxsizeincrement);
...
numcolumnsinshiftmatrix = size(interp_xaxis,1) / frameheight;
...
if toanalysevideo
    ...
    stabilisedvideoname = 'stabilizevideo.avi';
end
...
stabprog = waitbar(0,'Making a Stabilised Video');
...
for framecounter = 1:numbervideoframes
    if ~isempty(find(framecounter == framestostabilize))         %#ok<EFIND>
        ...
        if toanalysevideo
            videoinfo.CurrentTime = (framenumbertoadd-1)*(1/videoinfo.FrameRate);
        ...
        if ~blacklineflag
            rowsums = sum(stabilisedframe > 1,2);
            ...
            if ~isempty(blankrows)
                ...
                stabilisedframe(blankrows,:) = (stabilisedframe(nonblankrowtoaverage_prev,:) +...
                    stabilisedframe(nonblankrowtoaverage_next,:)) / 2;
                sumstabilisedframe(blankrows,:) = (sumstabilisedframe(nonblankrowtoaverage_prev,:) +...
                    sumstabilisedframe(nonblankrowtoaverage_next,:)) / 2;
            end
        end
        
        indiceswithnoimagedata = find(sumstabilisedframe < 1);
        
        sumstabilisedframe(indiceswithnoimagedata) = 1; %#ok<FNDSB>
        stabilisedframe = stabilisedframe ./ sumstabilisedframe;
        
        toscale = 1;
    else
        if maintaintimerelationships
            stabilisedframe = ones(stabilizedsize(2),stabilizedsize(1));
        else
            continue
        end
    ...
    if verbose
        ...
        set(stabfighandle,'Name',namestring);
        set(stabimghandle,'cdata',stabilisedframe);
    end
    
    stabilisedframe = stabilisedframe(imageborders(3):imageborders(4),...
    ...
    if toscale
        frametoadd = uint8(floor(scale(stabilisedframe) * 255) + 1);
    else
        frametoadd = uint8(stabilisedframe);
    end
...
if nargout >= 1
    varargout{1} = stabilisedvideoname;
end
