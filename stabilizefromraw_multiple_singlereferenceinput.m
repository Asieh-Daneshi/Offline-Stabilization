...
formatofstabframe = strcat('.',lower(formatofstabframe));
        referencediag=inputdlg('do you have a reference frame ready for all your videos? y=yes n=no');
         if referencediag{1,1}=='y'
             [refmatname, refmatpath] = uigetfile('*.tiff','referenceimage');
             referenceold=double(imread(char([refmatpath refmatname])));
             referenceoldxaddrand=size(referenceold,1)+60;
             referenceoldyaddrand=size(referenceold,2)+60;
             referencerand=randi(255,[referenceoldxaddrand referenceoldyaddrand]);
