...
% currentdir = 'E:\ausgelagerte_Studiendaten';
% currentdir = 'Z:\Bonn AOSLO\AOSLO\BAK8044R\2019_09_19_15_03_06_AOSLO';
...
for directorycounter = 1:numdirectories
    ...
    if tempdirname == 0
        disp('You pressed cancel instead of choosing a directory');
        warning('Continuing...');
        continue
    end
    ...
    for structurecounter = 1:length(dirstucture)
        if ~(dirstucture(structurecounter).isdir)
            ...
            if strcmp(fileextension,'AVI')
                if strcmp(tempname(end-13:end),'stabilized.avi')       % for excluding already stabilized videos from the list (extension = Bonn Lab)
                    %skip
                else
...
blinkthreshold = 25;  % 25; %(ND)   % This is a maximum change in mean pixel value between
...
minimummeanlevel = 1; % 8; %(ND)    % This is minimum mean pixel value that a frame has to
...
peakratiodiff = 0.1;   % 0.5;(ND)   % Maximum Change in ratio between the secondpeak and firstpeak
...
maxmotionthreshold = 0.05; % 0.3;(ND) % Maximum motion between frames, expressed as a percentage
...
badsamplethreshold = 0.9; % 0.7;(ND)% The threshold that is used to locate the strips that had
...
priorref_inputstruct = struct('samplerate',[],'vertsearchzone',55,'stripheight',9,...
    'badstripthreshold',0.6,'frameincrement',10,'minpercentofgoodstripsperframe',0.4,...
    'numlinesperfullframe',numlinesperfullframe); % Structure that contains input arguments for the makereference_priorref.m function.


%%
analyse_inputstruct = struct('samplerate',[],'vertsearchzone',55,'horisearchzone',[],...
    'startframe',1,'endframe',-1,'stripheight',9,'badstripthreshold',0.6,'minpercentofgoodstripsperframe',0.4,...
...
%% loop through all frames --> use each one as reference to calculate strip offsets
% blinkframes are skiped (choose video specific threshold in line 109)
GlobalReference = 1; % for this script (forDewarping) reference loops through all frames
        
% select frame/s from video
VObj1 = VideoReader(filelist{selection});
VidFrames = read(VObj1);
VidFrames = double(VidFrames(:,:,:));

blinkframes = getblinkframes(filelist{selection}, blinkthreshold,minimummeanlevel,blinkverbosity);
% framestouseasref = 1:size(VidFrames,3);
% framestouseasref = Selected_Frame;
% framestouseasref = 1;
framestouseasref = [42,43,110,155,158,159,160,161,168,171,217,218,243,253,254];
% framestouseasref(blinkframes) = [];

for frameNR = framestouseasref(1:end)       % select the frames to loop through as reference frames   
    ReferenceImage = VidFrames(:,:,frameNR);
    frameNR3 = num2str(frameNR, '%03d');
...
%             blinkframes = getblinkframes(videotoanalyse, blinkthreshold,minimummeanlevel,blinkverbosity);
    ...
    %         figure,imshow(finerefimage./255)
            ...
            load(analyseddatafilename,'analysedframes','frameshifts_strips','frameshifts_strips_spline','peakratios_strips',...
            ...
            % save variables for Dewarping seperately
            currfilename = filelist{selection};
            dewarpingresultsname = [currfilename(1:end-4),'_stabresult_ref',frameNR3,'.mat'];
            save(dewarpingresultsname,'analysedframes','frameshifts_strips','frameshifts_strips_spline');
            ...
            switch formatofstabframe
                case '.jpeg'
                    stabimagename = strcat(stabimagename_noext,'_ref',frameNR3,formatofstabframe);
                    imwrite(stabilizedmatrix(:,:,3) / 256,stabimagename,'Quality',100);
                case '.tiff'
                    stabimagename = strcat(stabimagename_noext,'_ref',frameNR3,'.tiff');
                    imwrite(stabilizedmatrix(:,:,3) / 256,stabimagename,'Compression','none');
                case '.gif'
                    stabimagename = strcat(stabimagename_noext,'_ref',frameNR3,'.gif');
    ...
    if tomakemontage
        ...
        switch formatofstabframe
            case '.jpeg'
                stabimagename = strcat(montagestring,'_ref',frameNR3,formatofstabframe);
                imwrite(montageimage / 256,stabimagename,'Quality',100);
            case '.tiff'
                stabimagename = strcat(montagestring,'_ref',frameNR3,'.tiff');
                imwrite(montageimage / 256,stabimagename,'Compression','none');
            case '.gif'
                stabimagename = strcat(montagestring,'_ref',frameNR3,'.gif');
