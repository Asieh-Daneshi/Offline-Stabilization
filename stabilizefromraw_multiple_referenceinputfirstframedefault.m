...
blinkthreshold = 1;                % This is a maximum change in mean pixel value between
                                    % frames before the function tags the frames as blink
                                    % frames.
minimummeanlevel = 1;              % This is minimum mean pixel value that a frame has to
...
numframestoaverage = -1;            % The number of frame to average toget if you want to remove the influence of
                                    ...
                                    % the frames
peakratiodiff = 0.1;   %from 0.05            % Maximum Change in ratio between the secondpeak and firstpeak
...
badsamplethreshold = 0.55;%from 0.55          % The thresholdthat is used to locate the strips that had
...
splineflag = 1;                     % When calculating splines during the stabilisation, we could calculate
                                    % splines for individual frames (splineflag = 1), or calculate on spline
                                    % for the full video (splineflag = 0).
stripheight=5;
vertsearchzone=59;
minpercentofgoodstripsperframe=0.4;
priorref_inputstruct = struct('samplerate',[],'vertsearchzone',vertsearchzone,'stripheight',stripheight,...
    'badstripthreshold',badsamplethreshold,'frameincrement',coarseframeincrement,'minpercentofgoodstripsperframe',minpercentofgoodstripsperframe,...
...
analyse_inputstruct = struct('samplerate',[],'vertsearchzone',vertsearchzone,'horisearchzone',[],...
    'startframe',1,'endframe',-1,'stripheight',stripheight,'badstripthreshold',badsamplethreshold,'minpercentofgoodstripsperframe',minpercentofgoodstripsperframe,...
...
correlationflags_analyse = [1;0];% Array that controls the cross-correlations conducted by analysevideo_priorref
...
% function.
analyverbosity = [1 0 0 0];         % The verbose array used by the analysevideo_priorref.m
% function.
...
for filecounter = 1:numfiletoanalyse
    try
        %referencediag=inputdlg('do you have a reference frame ready for this video in tiff format? y=yes n=no or enter number for frames to use as reference seperated by spaces');
        % if referencediag{1,1}=='y'
        %     [refmatname, refmatpath] = uigetfile('*.tiff','referenceimage');
        % end
        ...
        analyse_inputstruct.horisearchzone =  framewidth;
...
%         currenttexthandleindex = currenttexthandleindex + 1;
%             
%         [finereffilename, referenceimage] = makereference_priorref(finalname,...
%         coarsereffilename,blinkfilename, 'referenceimage', priorref_inputstruct,correlationflags_priorref,...
%         finerefverbosity);
%         set(texthandles(currenttexthandleindex - 1),'FontWeight','Normal');
%         set(texthandles(currenttexthandleindex),'FontWeight','Bold');
%         currenttexthandleindex = currenttexthandleindex + 1;
%     elseif referencediag{1,1}=='y'
%         cd(refmatpath);
%         referencerand=randi(255,[570 570]);
%         referenceold=double(imread(char(refmatname)));
%         fx=floor(size(referencerand,1)/2)-floor(size(referenceold,1)/2);
%         fy=floor(size(referencerand,2)/2)-floor(size(referenceold,2)/2);
%         referenceimage=referencerand;
%         for p=1:size(referenceold,1)
% 
%         for q=1:size(referenceold,2)
%                 referenceimage(fx+p,fy+q)=referenceold(p,q);     
%         end
%         end
%         finerefimagename=strcat(refmatpath,refmatname);
%         finereffilename=strcat(finerefimagename(1:end-5),'.mat');
%         save(finereffilename,'referenceimage');
%         set(texthandles(currenttexthandleindex - 1),'FontWeight','Normal');
%         set(texthandles(currenttexthandleindex),'FontWeight','Bold');
%         currenttexthandleindex = currenttexthandleindex + 1;
%     elseif isnan(str2double(referencediag{1,1}))==0
%         framenumforref=str2double(strsplit(referencediag{1,1}));
%         moviestructure=zeros(frameheight,framewidth,framenumforref);
%         for i=1:framenumforref
%             moviestructure(:,:,i)=readFrame(currentvideoinfo);
%         end
%         referenceold=moviestructure(:,:,i);
%         referencerand=randi(255,[570 570]);
%         fx=floor(size(referencerand,1)/2)-floor(size(referenceold,1)/2);
% 
%         fy=floor(size(referencerand,2)/2)-floor(size(referenceold,2)/2);
...
%         end
%         end
%         finereffilename=strcat(currentvideoinfo.Name(1:end-4),'.mat');
%         save(finereffilename,'referenceimage');
%         set(texthandles(currenttexthandleindex - 1),'FontWeight','Normal');
%         set(texthandles(currenttexthandleindex),'FontWeight','Bold');
%         currenttexthandleindex = currenttexthandleindex + 1;
%     else
%         fprintf('invalidselection');
%     end
        moviestructure=zeros(frameheight,framewidth,1);
        moviestructure(:,:,1)=readFrame(currentvideoinfo);
        referenceold=moviestructure(:,:,1);
        referencerand=randi(255,[570 570]);
        fx=floor(size(referencerand,1)/2)-floor(size(referenceold,1)/2);
...
%             finerefverbosity);
        ...
        save(analyseddatafilename,'blinkframes','goodframesegmentinfo','largemovementframes',...
            'stabilizedframe','stabilizedmatrix','stabilizedmatrix_full','-append');
